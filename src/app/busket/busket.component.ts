...
export class BusketComponent implements OnInit {
  ...
  readonly ROOT_URL =
    'http://localhost/interwebapi/api/Busket/Get/' + this.MyBusketID;
  ...
  getBusket() {
    ...
    this.http
      .get(this.ROOT_URL, { withCredentials: true })
  ...
  DeleteFromBusket(ITEM: Number) {
    this.http
      .delete('http://localhost/interwebapi/api/Busket/Delete/' + ITEM)
      .subscribe((res) => (this.s = res));
