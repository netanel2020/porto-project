...
export class ProdactsComponent implements OnInit {
  ...
  readonly ROOT_URL = 'http://localhost/interwebapi/api/Prodact/Get/';
  readonly ROOT_URLPostBusket =
    'http://localhost/interwebapi/api/Busket/addBusket/';
  ...
  constructor(
    ...
    private PostRequest: PostRequestService
  ) {}
  ...
  add_to_busket(items: any) {
    ...
    this.PostRequest.SavePost(
      this.BusketData,
      this.ROOT_URLPostBusket
    ).subscribe((data) => {
      ...
      console.log(
        'the prodact' + items.PRDname + 'is bin added to your busket sucssefoly'
      );
    });
  }
  ngOnInit(): void {
